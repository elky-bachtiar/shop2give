...
export class SupabaseTestUtils {
  ...
  private testUsers: Record<string, { email: string; password: string; token: string }> = {};
   ...
   * @param runSetupScript unused parameter kept for backwards compatibility
   */
  async startSupabase(runSetupScript = false): Promise<boolean> {
    try {
      ...
      const checkRunning = await this.checkSupabaseRunning();
      if (checkRunning) {
      ...
      // Try to start Supabase using the standard CLI
      console.log('Supabase is not running, attempting to start...');
  ...
  /**
   * Create or authenticate a test user and store their token
   * @param role The user role to create (e.g., 'campaign_owner', 'admin')
   * @returns Access token for the test user
   */
  async createOrAuthenticateTestUser(role: string = 'campaign_owner'): Promise<string | null> {
    if (!this.adminClient) {
      console.error('Admin client not available. Cannot create test users.');
      return null;
    }
    
    // Use role as key for storing users with different roles
    if (this.testUsers[role]?.token) {
      return this.testUsers[role].token;
    }
    
    // Create unique email for this role
    const testUser = {
      email: `test.${role}@example.com`,
      password: `Password123!test.${role}@example.com`
    };
    ...
    if (authError && authError.message.includes('Invalid login credentials')) {
      console.log(`Test user for role '${role}' does not exist. Creating...`);
      ...
      if (signInError) {
        console.error('Error signing in with new test user:', signInError);
        return null;
      }
    ...
    } else if (!authError && userData.session) {
  ...
  /**
   * Get list of deployed Supabase Edge Functions
   * @returns Array of function names
   */
  async getDeployedFunctions(): Promise<string[]> {
    try {
      // Check if we have a valid access token
      const accessToken = process.env.SUPABASE_ACCESS_TOKEN;
      
      // If no token or LOCAL_DEVELOPMENT is set, return hardcoded list of expected functions
      if (!accessToken || process.env.LOCAL_DEVELOPMENT === 'true') {
        console.log('No Supabase access token found or in local development - using hardcoded function list');
        return [
          'generate-csrf-token',
          ...
          'stripe-webhook'
          // Add other expected functions here
        ];
      }
      
      // Try to get the list via CLI first
      try {
        const { stdout } = await execAsync('supabase functions list', { cwd: projectRoot });
        const functions = stdout
          .split('\n')
          .filter(line => line.trim() && !line.startsWith('[') && !line.includes('Functions'))
          .map(line => line.trim());
        ...
        console.log('Could not get functions from CLI, falling back to hardcoded list');
        return [
          'generate-csrf-token',
          'seed-example-campaigns',
          'seed-example-products', 
      ...
      // Fallback to checking known functions via HTTP
      ...
      // Try to detect which functions exist by making HTTP requests
      const availableFunctions: string[] = [];
      
      for (const funcName of knownFunctions) {
        try {
          // We just check if the endpoints exist, not if they work
          // Use fetch to make a simple HEAD request
          const response = await fetch(`${testConfig.supabaseUrl}/functions/v1/${funcName}`, {
            method: 'HEAD'
          });
          
          // If we get any response (even 401), the function exists
          if (response.status !== 404) {
            availableFunctions.push(funcName);
          }
        } catch (error) {
          // Ignore errors, just means function doesn't exist or network issue
        }
      }
      
      return availableFunctions.length > 0 ? availableFunctions : knownFunctions;
    } catch (error) {
      console.error('Error getting deployed functions:', error);
      // Return hardcoded functions as fallback
      return [
        'generate-csrf-token',
        'seed-example-campaigns',
        'seed-example-products', 
