...
export function createRateLimiter(
  ...
  return new RateLimiter({} as any, "rate_limits").setConfig({
    name: functionName,
    identifier,
    maxRequests,
    windowSeconds
...
interface SimpleRateLimitResult {
  limited: boolean;
  remaining: number;
  reset: number;
}
...
/**
 * Creates a simple in-memory rate limiter (no database dependency)
 * @param maxRequests - Maximum number of requests allowed
 * @param windowMs - Time window in milliseconds
 * @returns Rate limiter function
 */
export function createSimpleRateLimiter(maxRequests = 10, windowMs = 60000): (clientId: string) => SimpleRateLimitResult {
  const clients = new Map<string, { count: number; timestamp: number }>(); 
  
  return function rateLimit(clientId: string): SimpleRateLimitResult {
    const now = Date.now();
